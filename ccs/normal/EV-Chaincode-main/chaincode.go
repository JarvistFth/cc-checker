 ...
 * Aangevuld door Joeri Heyvaert en Paul pbhevelenbos i.k.v. S3 Blockchain Project/HOWEST
 * Datum: 10/01/2021
 */
...
func (cc *Chaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if fn == "procespbDR" {
		result, err = procespbDR(stub, args)
	}
...
func procespbDR(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	evDriver := Stakeholder{}
	if err != nil {
		return "", fmt.Errorf("procespbDR failed retrieving evDriver with error: %s", err)
	}

	if evDriverAsBytes == nil {
		return "", fmt.Errorf("procespbDR failed retrieving evDriver: %s", evdrContractId)
	}
	...
	cpo := Stakeholder{}
	if err != nil {
		return "", fmt.Errorf("procespbDR failed retrieving CPO with error: %s", err)
	}

	if cpoAsBytes == nil {
		return "", fmt.Errorf("procespbDR failed retrieving CPO: %s", cpoContractId)
	}
	...
	if err != nil {
		return "", fmt.Errorf("procespbDR failed retrieving FI with error: %s", err)
	}

	if fiAsBytes == nil {
		return "", fmt.Errorf("procespbDR failed retrieving FI: %s", contractIdFI)
	}
	...
	eMSP := Stakeholder{}
	if err != nil {
		return "", fmt.Errorf("procespbDR failed retrieving eMSP with error: %s", err)
	}

	if emspAsBytes == nil {
		return "", fmt.Errorf("procespbDR failed retrieving eMSP: %s", args[3])
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed to set CDR in procespbDR: %s", recordId)
	}

	err = stub.PutState(recordId, cdrJSONasBytes)

	if err != nil {
		return "", fmt.Errorf("failed to put state of cdr in procespbDR: %s", recordId)
	}
