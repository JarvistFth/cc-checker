...
// 初始化审计合约
func InitMain(context contract2.Context) *contract2.Response {
	maintainers, err := parseMaintainers(context.GetArgs(), context)
	if err != nil {
		contract2.Error(fmt.Sprint("解析合约运维人员出错，详细信息："))
	}
	// 存储审计运维人员信息
	for _, v := range maintainers {
		if err := record.StoreItem(v, context); err != nil {
			return contract2.Error(fmt.Sprintf("运维人员'%s'信息存储出错，详细信息：%s",
				v.Name, err.Error()))
		}
	}
	// 存储审计运维人员个数
	err = context.PutState(orgnization.MaintainerCountKey, []byte(strconv.FormatUint(uint64(
		len(maintainers)), 10)))
	if err != nil {
		return contract2.Error(fmt.Sprintf("运维人员个数信息存储出错，详细信息：%s",
			err.Error()))
	}
...
// 从输入参数中解析包括哪些审计维护人员
func parseMaintainers(args []string, context contract2.Context) (
...
// 从输入参数中解析单个审计维护人员
func parseSingleMaintainer(arg string, index uint32, context contract2.Context) (
	*orgnization.Maintainer, error) {
