...
func (stub *MockStub) DelState(key string) error {
	log.Debug("MockStub", stub.Name, "Deleting", key, stub.State[key])
...
func (iter *MockStateRangeQueryIterator) HasNext() bool {
	if iter.Closed {
		...
		log.Debug("HasNext() but already closed")
		return false
	}

	if iter.Current == nil {
		log.Error("HasNext() couldn't get Current")
		return false
	}
	...
	for current != nil {
		...
		if comp1 >= 0 {
			if comp2 < 0 {
				log.Debug("HasNext() got next")
				return true
			} else {
				log.Debug("HasNext() but no next")
				return false
...
func (iter *MockStateRangeQueryIterator) Print() {
	log.Debug("MockStateRangeQueryIterator {")
	log.Debug("Closed?", iter.Closed)
	log.Debug("Stub", iter.Stub)
	log.Debug("StartKey", iter.StartKey)
	log.Debug("EndKey", iter.EndKey)
	log.Debug("Current", iter.Current)
	log.Debug("HasNext?", iter.HasNext())
	log.Debug("}")
}

func NewMockStateRangeQueryIterator(stub *MockStub, startKey string, endKey string) *MockStateRangeQueryIterator {
	log.Debug("NewMockStateRangeQueryIterator(", stub, startKey, endKey, ")")
...
func getFuncArgs(bytes [][]byte) (string, []string) {
	log.Debugf("getFuncArgs(%x)", bytes)
	function := string(bytes[0])
	args := make([]string, len(bytes)-1)
	for i := 1; i < len(bytes); i++ {
		log.Debugf("getFuncArgs - i:%x, len(bytes):%x", i, len(bytes))
